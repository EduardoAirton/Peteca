        ...
        VStack(spacing: 20) {
            ...
            HStack{
                ForEach(0..<3) { itens in
                    petecaBuild(imagem: "\(self.ordemPeteca[itens])", onChanged: self.petecaMoved, pena: false)
                    .overlay(
                        Color.clear
                    )
                    .zIndex(10)
                ...
                ForEach(0..<4) { itens in
                    petecaBuild(imagem: "\(self.ordemPena[itens])",onChanged: nil ,pena: true)
                    .zIndex(1)
                }
            }
        }
...
public struct Peteca: View {
    ...
    public init(playgroundView: PlaygroundView, posicaoPeteca: CGPoint, tamanho: CGFloat, imagem: String, petecaBuild: Bool) {
        self.playgroundView = playgroundView
        self.posicaoPeteca = posicaoPeteca
        self.tamanho = tamanho
        self.imagem = imagem
        self.petecaBuild = petecaBuild
    }
